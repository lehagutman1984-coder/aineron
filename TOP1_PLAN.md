...
> **Составлен:** 2026-06-21 | **Обновлён:** 2026-06-24 | Модель: Claude Sonnet 4.6
...
| 5 | Image-to-image (фото + промт → image-модель) | Недоделка | Code-complete `handlers/img2img_cmd.py` (64ee285). Требует верификации на проде (нужна img2img-модель в каталоге) |
...
| P1 | **Image-to-image** (фото → редактирование) | Code-complete `/img2img` FSM (64ee285). Нужна модель с `requires_input_images=True` в каталоге |
...
| ~~P3~~ | ~~**AI-стикеры**~~ (генерация стикерпака через /sticker) | ✅ `/sticker` FSM, конвертация 512×512 PNG (574f348) |
...
| ~~**Канбан-дашборд проектов**~~ | ✅ Drag&drop Active/Paused/Done (ea1ff51) |
| ~~**UI базы знаний**~~ | ✅ Загрузка/просмотр документов RAG в проекте |
| ~~**Оптимистичный UI**~~ | ✅ Instant send + prefetch /models в sidebar (bd55965) |
...
1. Code-complete **Image-to-image** (`/img2img` FSM handler в боте) — верификация: нужна img2img модель в каталоге
...
### Спринт 3 — Дифференциация и B2B (в процессе)

1. **RAG / база знаний** (pgvector, загрузка документов в Project, UI в Next.js) — *следующий*
2. ✅ **Единый `UsageEvent`** — слияние веб- и бот-аналитики (Sprint 2)
3. ✅ **Группы/каналы** как AI-ассистент + оргбиллинг через Telegram — TelegramGroup + /reggroup (4b4ecd8)
4. ✅ **AI-агенты/сценарии** — `/ai` команда: пост, email, бриф, код-ревью, summary, перевод (7b8566e)
5. ✅ **A/B тестирование промтов** — `PromptABTest`, интеграция в StreamMessageView, аналитика (7b8566e)
6. ✅ **Ежедневный AI-дайджест** — `/digest`, Celery beat, настройка времени (bf8c964)
7. ✅ **Редактирование/удаление сообщений в боте** — ✏️🗑️ кнопки, FSM (53937b1)
8. ✅ **Robokassa в боте** — выбор пакета → URL → оплата + Telegram-уведомление (0cb8e16)
9. ✅ **img→video** — /img2video FSM: фото + промт → видео через Kling/Veo (8861c3b)
10. ✅ **Канбан-дашборд проектов** — drag&drop статусы Active/Paused/Done (ea1ff51)
