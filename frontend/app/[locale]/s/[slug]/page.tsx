...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const t = await getTranslations("publicSpace");
  const brand = brandName();
  if (!space) return { title: t("notFoundMetaTitle") };
  return {
    title: t("metaTitle", { name: space.name, brand }),
    description: space.system_prompt
      ? space.system_prompt.slice(0, 160)
      : t("metaDescriptionFallback", { name: space.name, brand }),
    openGraph: {
      title: `${space.name} — Project Space`,
      description: space.system_prompt?.slice(0, 160) ?? t("ogDescriptionFallback", { brand }),
      siteName: brand,
      type: "website",
    },
  };
...
function formatSize(bytes: number, t: Awaited<ReturnType<typeof getTranslations>>): string {
  if (bytes < 1024) return t("sizeBytes", { count: bytes });
  if (bytes < 1024 * 1024) return t("sizeKb", { count: (bytes / 1024).toFixed(1) });
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#f7f7f8]">
        <div className="text-center">
          ...
          <p className="mt-1 text-[15px] text-[rgba(13,13,13,0.45)]">{t("notFoundHint")}</p>
    ...
    <div className="min-h-screen bg-[#f7f7f8]">
      ...
      <header className="border-b border-[rgba(13,13,13,0.08)] bg-white">
        <div className="mx-auto flex max-w-[760px] items-center justify-between px-4 py-3">
          ...
          <span className="text-[13px] text-[rgba(13,13,13,0.40)]">{t("headerLabel")}</span>
        </div>
      </header>
        ...
        <div className="mb-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-0.5 flex items-center gap-1.5 text-[14px] text-[rgba(13,13,13,0.40)]">
              <Clock size={11} />
              {t("createdOn", { date: formatDate(space.created_at, locale) })}
            </p>
          </div>
        </div>
        ...
        {space.public_show_files && space.files.length > 0 && (
          <section className="mb-6 rounded-[14px] border border-[rgba(13,13,13,0.09)] bg-white p-5">
            <div className="mb-3 flex items-center gap-2">
              ...
              <span className="ms-auto rounded-full bg-[rgba(13,13,13,0.07)] px-2 py-0.5 text-[13px] font-medium text-[rgba(13,13,13,0.50)]">
                {t("filesCount", { count: space.files.length })}
              </span>
            </div>
                ...
                <div key={f.id} className="flex items-center justify-between py-2.5">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <span className="shrink-0 rounded-[4px] bg-[rgba(13,13,13,0.07)] px-1.5 py-0.5 text-[12px] text-[rgba(13,13,13,0.45)]">
                      {fileTypeLabel[f.file_type] ?? t("fileTypeOther")}
                    </span>
                  </div>
                  <span className="ms-3 shrink-0 text-[14px] text-[rgba(13,13,13,0.40)]">
                    {formatSize(f.file_size, t)}
                  </span>
                </div>
              ))}
            </div>
            ...
            <div className="divide-y divide-[rgba(13,13,13,0.07)]">
              {space.chats.map((c) => (
                <div key={c.id} className="flex items-center justify-between py-2.5">
                  <span className="truncate text-[15px] text-[#1A1A1A]">{c.title || t("untitledChat")}</span>
                  <span className="ms-3 shrink-0 text-[14px] text-[rgba(13,13,13,0.40)]">
