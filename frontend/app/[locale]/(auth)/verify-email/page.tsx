...
function VerifyEmailForm() {
  ...
  return (
    <div className="rounded-[16px] border border-[rgba(13,13,13,0.10)] bg-white p-8 shadow-sm text-center">
      ...
      <h1 className="mb-1 text-[22px] font-bold text-[#1A1A1A]">{t("verifyTitle")}</h1>
      <p className="mb-6 text-[16px] text-[rgba(13,13,13,0.55)]">
        {t("verifySentTo")}{" "}
        <span className="font-medium text-[#1A1A1A]">{user?.email ?? t("verifyYourEmail")}</span>
      </p>
      ...
      >
        {loading ? t("verifyChecking") : t("verifyConfirm")}
      </button>
      ...
      >
        {resendCountdown > 0
          ? t("verifyResendIn", { seconds: resendCountdown })
          : resendLoading
          ? t("verifySending")
          : t("verifyResend")}
      </button>
