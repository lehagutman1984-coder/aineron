...
export default function ChatPage() {
  ...
  // Sprint 3: запускаем polling для сообщения «в работе» при загрузке/навигации.
  // Нужно для img2video (переход из "Оживить" в новый чат), для перезагрузки
  // страницы во время генерации, и для первого текстового сообщения в только
  // что созданном чате (createChat ставит Celery-задачу и сразу редиректит —
  // до этого фикса статус текстовых ответов не опрашивался вообще: гонка
  // была ограничена только provider === "fal-ai", хотя нужна любому
  // провайдеру — иначе прогресс/результат не появятся сами, пока что-то
  // ещё не дёрнет рефетч чата).
  useEffect(() => {
  ...
  return (
    ...
    <div className="flex h-full min-w-0 flex-1 flex-col">
          ...
          <div className="relative group/export">
            ...
            <div className="absolute right-0 top-9 z-20 hidden group-hover/export:block w-32 rounded-[8px] border border-[rgba(13,13,13,0.10)] bg-white shadow-lg overflow-hidden">
              <a
                ...
                download
                className="flex w-full items-center gap-2 px-3 py-2 text-start text-[14px] text-[rgba(13,13,13,0.70)] hover:bg-[rgba(13,13,13,0.04)]"
              >
                <FileText size={12} />
                Markdown
              </a>
      ...
      <div className="flex-1 overflow-y-auto" ref={scrollContainerRef} onScroll={handleScroll}>
              ...
              <div className="mt-4 grid w-full grid-cols-1 gap-2.5 sm:grid-cols-2">
                {getStarterPrompts(chat.network, t).map((card, i) => (
                  <button
                    ...
                    disabled={isBusy}
                    className="group rounded-[12px] border px-4 py-3.5 text-start transition-all disabled:cursor-not-allowed disabled:opacity-40"
                    style={{
                      background: "var(--chat-surface)",
        ...
        {showScrollBtn && (
          <div className="sticky bottom-4 flex justify-end pe-6 pb-1">
            <button
        ...
        <form onSubmit={handleSubmit} className="mx-auto max-w-2xl">
          ...
          >
            ...
            <textarea
              ...
              rows={1}
              disabled={isBusy}
              className="block w-full resize-none bg-transparent px-4 py-3.5 pe-24 text-[16px] leading-relaxed text-[#1A1A1A] outline-none disabled:opacity-50 dark:text-[#EDE8E3] dark:placeholder:text-[rgba(236,236,236,0.35)]"
          ...
          {chat.network.provider === "fal-ai" && (
            <div className="mt-1.5 flex items-center gap-1 px-1">
              ...
              {batchCount > 1 && (
                <span className="ms-1 text-[12px] text-[rgba(13,13,13,0.35)] dark:text-[rgba(236,236,236,0.3)]">
...
function StreamingDisplay({ text }: { text: string }) {
  ...
  return (
    ...
    >
      ...
      <span
        className="ms-0.5 inline-block animate-pulse"
        style={{
          width: "2px",
          height: "1.1em",
          background: "#D97757",
          ...
          {sources.map((s) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {s.path !== s.filename && (
                  <span className="ms-auto shrink-0 text-[12px] text-[rgba(13,13,13,0.35)] dark:text-[rgba(236,236,236,0.3)] font-mono truncate max-w-[180px]" title={s.path}>
                    {s.path}
                  </span>
                )}
              </div>
              {s.snippet && (
                <div className="hidden group-hover/src:block ms-5 text-[13px] leading-relaxed text-[rgba(13,13,13,0.45)] dark:text-[rgba(236,236,236,0.38)] line-clamp-2">
                  {s.snippet}
                </div>
              )}
            </div>
