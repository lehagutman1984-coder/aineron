...
export default function TasksPage() {
  const t = useTranslations("accountTasks");
  ...
  const toggleMutation = useMutation({
    mutationFn: (task: AITask) => patchTask(task.id, { is_active: !task.is_active }),
    onSuccess: invalidate,
    onError: (err) =>
      setError(err instanceof APIError ? err.message : t("errorUpdate")),
  });
  ...
  const runMutation = useMutation({
    ...
    onSuccess: () => setNotice(t("noticeRun")),
    onError: (err) =>
      setError(err instanceof APIError ? err.message : t("errorRun")),
  });
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      <div className="mb-2 flex items-center justify-between">
        <h1 className="text-[22px] font-bold text-[#1A1A1A]">{t("pageTitle")}</h1>
        {data && (
          <span className="text-[14px] text-[rgba(13,13,13,0.45)]">
            {t("activeCount", { count: data.active_count, limit: data.limit })}
          </span>
        )}
      </div>
        ...
        <div className="flex flex-col gap-2">
          {tasks.map((task) => (
            <TaskRow
              ...
              onRun={() => { setNotice(null); setError(null); runMutation.mutate(task.id); }}
            />
          ))}
        </div>
      )}
      ...
      <div className="flex items-start gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-[13px] text-[rgba(13,13,13,0.4)]">
            {task.schedule_human} · {status} · {t("runsCount", { count: task.runs_count })}
          </p>
        </div>
        <div className="flex shrink-0 items-center gap-1.5">
          ...
          <button
            onClick={onToggle}
            title={task.is_active ? t("pauseTitle") : t("enableTitle")}
            className="flex h-7 w-7 items-center justify-center rounded-[6px] text-[rgba(13,13,13,0.35)] hover:bg-[rgba(13,13,13,0.05)] hover:text-[#1A1A1A] transition-all"
          >
...
function CreateTaskForm({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: onDone,
    onError: (err) =>
      onError(err instanceof APIError ? err.message : t("errorCreate")),
  });
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-3">
        ...
        >
          {createMutation.isPending ? t("submitCreating") : t("submitCreate")}
        </button>
        <button
          type="button"
          onClick={onCancel}
