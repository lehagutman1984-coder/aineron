...
function Section({
  ...
  return (
    ...
    >
      <h2 className="mb-4 text-[20px] font-semibold text-[#1A1A1A]">{title}</h2>
      {children}
    </section>
  );
}
    ...
    <div className="min-h-screen bg-[#FAF9F7] px-4 py-12">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <Section id="cursor" title="Cursor">
          ...
          <div className="space-y-4">
            <Step n={1}>
              ...
              <strong className="text-[#1A1A1A]">Models</strong> → кнопка{" "}
              <strong className="text-[#1A1A1A]">Add Model</strong>.
            </Step>
            <Step n={2}>
              Прокрутите до раздела <strong className="text-[#1A1A1A]">OpenAI API Key</strong>,
              нажмите <strong className="text-[#1A1A1A]">Override OpenAI Base URL</strong> и
              введите <Code>https://aineron.ru/api/v1</Code>. В поле{" "}
              <strong className="text-[#1A1A1A]">API Key</strong> введите ваш ключ{" "}
              <Code>ak_...</Code>.
            </Step>
            <Step n={3}>
              <div>
                В поле <strong className="text-[#1A1A1A]">Model Name</strong> введите
            ...
            <Step n={4}>
              Нажмите <strong className="text-[#1A1A1A]">Save</strong>. В правом нижнем углу
              редактора выберите добавленную модель и откройте чат (<Kbd>Ctrl+L</Kbd>). Отправьте
              тестовое сообщение — готово.
            </Step>
          </div>
          <AlertInfo>
            Cursor кэширует ответы для автодополнения. При нехватке средств автодополнение временно
        ...
        <Section id="cline" title="Cline">
          ...
          <div className="space-y-4">
            <Step n={1}>
              Найдите <strong className="text-[#1A1A1A]">Cline</strong> в маркетплейсе VS Code и
              установите. После этого появится иконка в левой панели.
            </Step>
            <Step n={2}>
              <div>
                Откройте настройки Cline → раздел{" "}
                <strong className="text-[#1A1A1A]">API Provider</strong> → выберите{" "}
                <strong className="text-[#1A1A1A]">OpenAI Compatible</strong>. Заполните поля:
              </div>
              <div className="mt-3">
            ...
            <Step n={3}>
              <div>
                В поле <strong className="text-[#1A1A1A]">Model ID</strong> введите любой
        ...
        <Section id="continue" title="Continue">
          ...
          <div className="space-y-4">
            <Step n={1}>
              Установите <strong className="text-[#1A1A1A]">Continue</strong> из маркетплейса VS
              Code или JetBrains. После установки откройте файл конфигурации:{" "}
              <Code>~/.continue/config.json</Code> (или через команду{" "}
              <strong className="text-[#1A1A1A]">Continue: Open Config</strong>).
            </Step>
            <Step n={2}>
              <div>
        ...
        <section className="rounded-[16px] border border-[rgba(13,13,13,0.10)] bg-white p-6 shadow-sm">
          ...
          <div className="space-y-5">
            {[
              ...
              {
                q: "Как проверить остаток баланса?",
              ...
              },
              {
                ...
                a: (
                  <>
                    ...
                    <Code>gpt-4o-mini</Code>), чтобы снизить задержку и расход средств.
                  </>
                ),
              },
            ].map((item, i) => (
