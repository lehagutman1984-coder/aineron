...
### Реальное списание доплаты включено на aineron.ru — claude-fable-5 (2026-08-06)

Founder-решение: первой моделью для реального overage выбрана **Fable 5**, не
Opus 5 из первоначального плана §6 — на момент раскатки Opus 5 оказалась
нестабильна у обоих провайдеров (см. находки E2E-теста ниже), Fable 5 живьём
подтверждена рабочей. `TOKEN_OVERAGE_ENABLED=1`, `TOKEN_OVERAGE_DRY_RUN=0`,
`TOKEN_OVERAGE_MODELS=claude-fable-5`, `TOKEN_OVERAGE_SETTLE_FROM=<момент включения>`.

**Перед включением — полный E2E-прогон на живом проде** (новая регистрация,
подтверждение email, вход/выход, чат), обязательное условие от founder.
По ходу теста найдены и исправлены 4 реальных бага (все — до включения
реального списания, кроме находки о рефанде, обнаруженной уже после):

1. **Фолбэк не ловил "успешный", но нечитаемый ответ провайдера** — apimart
   иногда возвращает `str` вместо `ChatCompletion`, `_run()` в `providers.py`
   считал это успехом, ошибка проявлялась только в `tasks.py` вне зоны
   видимости фолбэка. Коммит `6969e2c`.
2. **Провайдер шлёт SSE-чанки на не-стриминговый запрос** — тот же класс
   проблемы, другое проявление: `apimart` иногда отвечает `data: {...}`
   чанками даже без `stream=True`. Раньше фатально; теперь чанки собираются
   в текст вручную. Тот же коммит `6969e2c`.
3. **Сырой текст исключения утекал пользователю** при провале генерации на
   Celery-пути (бот + `/v1/chats/`) — веб-SSE-путь уже был защищён, дыра
   была только здесь. Коммит `88391de`.
4. **Возврат pre-charge не выполнялся** в ветке `free_model_deprecated` —
   `return` там срабатывал раньше общего блока возврата средств. Условие
   ловит по вхождению `'free model'`/`'deprecated'` в текст ошибки
   провайдера — задевает и ПЛАТНЫЕ модели при случайном совпадении текста
   (реальный кейс: `claude-fable-5`). Найдены и вручную возвращены 2 реальных
   пострадавших списания за 30 дней: `kursnew@bk.ru` (`claude-opus-5`, 22 ₽,
   2026-08-04) и qa-тест (`claude-fable-5`, 15 ₽). Коммит `536ed9e`.

**Отдельно, не баг кода — операционная находка:** nginx резолвит upstream
`daphne` при СВОЁМ старте и кэширует IP; точечный `docker-compose restart
daphne` (без пересоздания nginx) даёт новый IP контейнеру — nginx продолжает
стучаться в старый, чат-SSE падает 502 (`connect() failed (111: Connection
refused)`). После любого точечного рестарта `web`/`daphne`/`frontend` теперь
следом рестартуем `nginx` — тот же паттерн, что уже был в `deploy_intl.sh`
для `.net`, здесь просто не соблюдался при точечных (не через `deploy.sh`)
передеплоях в этой сессии.

**Отдельно, не баг — живая проблема провайдера:** `claude-opus-5` на момент
теста была недоступна у `laozhang` целиком («No available channel», 3/3
попыток) и нестабильна у `apimart`-фолбэка (то не тот формат ответа, то
пустой контент при списанных деньгах). Это и стало причиной выбрать Fable 5
первой моделью вместо Opus 5.

**Подтверждение работы live на проде:**
- Короткое сообщение — без доплаты, поведение не изменилось.
- Сообщение на 2500+ слов (реально 10218 токенов вывода) — `cost_kopecks=4122`,
  `overage_kopecks`/`settled_kopecks=4000` (упёрлось в `ABS_CAP_KOPECKS`),
  реальная запись `BalanceTransaction(type='overage', reference='overage:3199')`,
  баланс списан корректно (флаг 15₽ + доплата 40₽), **чек показан в
  интерфейсе**: «Длинный ответ · 10 655 токенов · доплата 40 ₽» под ответом.

### То же самое включено на aineron.net (2026-08-06)

Тот же набор флагов (`claude-fable-5`, `TOKEN_OVERAGE_SETTLE_FROM=<момент
включения>`), тот же порядок: сначала полный E2E на живом `.net` (регистрация
— без шага подтверждения email, в отличие от `.ru`, не баг, отдельная
особенность INTL-потока, не расследовали дальше; вход/выход; чат) — все три
коммита-фикса из E2E-теста `.ru` подтянуты и на `.net` тем же `git pull` +
рестарт `web/celery_worker/celery_beat/daphne` + рестарт `nginx` (тот же урок
про кэш IP).

Живое подтверждение: короткое сообщение — без доплаты; длинное (2500+ слов,
реально 8376 токенов вывода) — `cost_kopecks=3368`, `overage_kopecks`/
`settled_kopecks=4000` (тот же `ABS_CAP_KOPECKS`), реальная запись
`BalanceTransaction(type='overage', reference='overage:39')`, чек в
интерфейсе на английском: «Long response · 8,594 tokens · extra charge
4,000 credits» — корректная локаль и валюта витрины (credits, не ₽).

**Оба инстанса теперь на паритете: Спринты 0-4 в проде, реальное списание
доплаты живёт на `claude-fable-5` на обоих, остальные 6 аудированных моделей
остаются dry-run до отдельного решения по раскатке дальше.**

### Вторая модель раскатки: claude-opus-4-8 (2026-08-06/07)

Следующая по приоритету из исходного порядка плана (после Opus 5, которая
по-прежнему нерабочая). Перед включением — та же дисциплина: живой тест на
`.ru` и `.net` (длинный ответ без доплаты, только чтобы подтвердить, что
модель отвечает корректно) до флипа флага.

Живьём подтверждено на обоих: провайдер `laozhang` регулярно уходит в таймаут
или 503 именно на этой модели (не только на Opus 5), фолбэк на `apimart`
отрабатывает исправно (включая наш фикс SSE-чанков — сработал живьём на
`.net`: «apimart отдал SSE-чанки на не-стриминговый запрос — собрано вручную,
фолбэк не потребовался»). После включения `TOKEN_OVERAGE_MODELS=claude-fable-5,claude-opus-4-8`
на обоих инстансах: реальная доплата списана и на `.ru` (7858 токенов вывода,
доплата 5,47 ₽, `overage:3203`), и на `.net` (15512 токенов вывода в
финальном тесте через живой SSE в существующем чате, доплата 784 credits,
`overage:45`), чек в интерфейсе подтверждён на обоих: «Long response ·
15,512 tokens · extra charge 784 credits».

**Важная методологическая находка при этом тесте:** чек Спринта 4 показывается
**только** для сообщений, отправленных в уже существующий чат через SSE
(`POST /messages/stream/`) — это осознанное ограничение фронтенда (поле
`overage_billing` клиентское, не персистится). Первое сообщение нового чата
идёт через `POST /v1/chats/` (Celery-путь, без SSE) — доплата списывается
корректно (проверено по `MessageTokenUsage`/`BalanceTransaction`), но чек в
интерфейсе не показывается для этого конкретного сообщения. Не баг, просто
через какой путь тестировать чек — важно для будущих раскаток.

**Раскатано: `claude-fable-5`, `claude-opus-4-8`. Осталось dry-run: `claude-opus-5`
(нерабочая у провайдера), `gpt-5-pro`, `gpt-5.4-pro`, `gpt-5.6-terra`, `gpt-5.3`
(нерабочая у провайдера).**

### `gpt-5-pro` — раскатано 2026-08-07

Третья модель. При живом тесте перед раскаткой найден и исправлен реальный
баг фолбэка (не специфичный для overage — влиял на все сообщения gpt-5-pro,
включая бесплатные от доплаты): laozhang для этой модели иногда отдаёт 400
`"Invalid 'max_output_tokens': integer below minimum value... got 0"` —
похоже, транслирует запрос во внутренний `v1/responses`-формат и теряет
параметр. Мы всегда шлём валидный `max_tokens` (проверено: `auto_max_tokens
('gpt-5-pro')=16384`, модель не входит в `TOKEN_OVERAGE_MODELS` на момент
бага → `preflight_max_tokens` не мог быть причиной) — это ошибка на стороне
прокси laozhang, не нашего кода. `is_availability_error()` не распознавал
этот паттерн 400-ошибки как fallback-worthy, поэтому запрос падал вместо
переключения на `apimart`. Исправлено в `providers.py` (коммит `681cfbc`) —
добавлен маркер на `max_output_tokens`/`max_tokens` + `below minimum value`/
`integer_below_min_value`/`got 0`; регрессионный тест в `test_providers.py`
(`test_max_output_tokens_below_minimum_falls_back`). Задеплоено на оба
инстанса до включения доплаты.

После фикса живьём подтверждено на `.ru`: короткое сообщение и статья на
~1200 слов прошли через фолбэк на `apimart` без ошибок (laozhang в моменте
отдавал то тот же 400 max_output_tokens, то отдельный 404 "only supported in
v1/responses" — оба случая корректно фолбечат). Отдельно отмечено: генерация
через `apimart` для этой модели медленная (наблюдались ответы 130–450 сек до
завершения не-стримингового реконструированного запроса) — не баг нашего
кода, особенность reasoning-модели у fallback-провайдера; один тестовый
прогон на `.ru` (статья 3000 слов) оборвался по `httpx.RemoteProtocolError`
(apimart закрыл соединение раньше отправки полного тела, наш `_CHAT_TIMEOUT`
read=90с не является причиной — обрыв случился на стороне apimart значительно
позже 90с) — списание корректно вернулось пользователю (refund сработал),
повторный запрос с тем же промтом прошёл успешно. Для будущих раскаток
reasoning-моделей с длинными ответами через фолбэк — закладывать так же
широкий запас по времени ожидания в тестах, не считать единичный обрыв
блокером при наличии рабочего refund-пути.

После включения `TOKEN_OVERAGE_MODELS=claude-fable-5,claude-opus-4-8,gpt-5-pro`
на обоих инстансах: реальная доплата списана и на `.ru` (14 102 токена
вывода, доплата 120 ₽, `overage:3215`), и на `.net` (12 391 токен вывода,
доплата 120 credits — сработал cap, `overage:49`), чек в интерфейсе
подтверждён на обоих: «Длинный ответ · 18 102 токенов · доплата 120 ₽» (.ru)
и «Long response · 12,769 tokens · extra charge 12,000 credits» (.net).

**Раскатано: `claude-fable-5`, `claude-opus-4-8`, `gpt-5-pro`. Осталось
dry-run: `claude-opus-5` (нерабочая у провайдера), `gpt-5.4-pro`,
`gpt-5.6-terra`, `gpt-5.3` (нерабочая у провайдера).**

### `gpt-5.6-terra` — раскатано 2026-08-07

Четвёртая модель. Живой тест перед раскаткой сразу вскрыл вариацию того же
бага прокси laozhang, найденного на gpt-5-pro — но с ДРУГИМ именем параметра:
400 `"Invalid 'max_completion_tokens': integer below minimum value... got
0"` (не `max_output_tokens`). Прежний точечный матчинг по имени параметра в
`is_availability_error()` это не ловил. Исправлено обобщением матчинга —
теперь ищем `token` + (`param`/`invalid`) + маркеры "below minimum"/
"integer_below_min_value"/"got 0", не привязываясь к конкретному имени
параметра (коммит `47d9b48`, регрессионный тест
`test_max_completion_tokens_below_minimum_falls_back`). Задеплоено на оба
инстанса до включения доплаты.

После фикса живьём подтверждено на обоих инстансах: короткое сообщение и
статья на ~6000+ слов (запрошенный максимум, модель гарантированно упирается
в потолок `model_max_tokens_cap('gpt-5.6-terra')=16384`) прошли через фолбэк
на apimart без ошибок. Модель заметно быстрее gpt-5-pro (apimart отвечает
за секунды, а не минуты) — категория "fast", не reasoning-модель.

После включения `TOKEN_OVERAGE_MODELS=claude-fable-5,claude-opus-4-8,
gpt-5-pro,gpt-5.6-terra` на обоих инстансах: реальная доплата списана и на
`.ru` (14 226 токенов вывода, доплата 5,01 ₽, `overage:3225`), и на `.net`
(14 866 токенов вывода, доплата 614 credits, `overage:53`), чек в интерфейсе
подтверждён на обоих: «Длинный ответ · 14 226 токенов · доплата 5,01 ₽» (.ru)
и «Long response · 14,866 tokens · extra charge 614 credits» (.net).

**Раскатано: `claude-fable-5`, `claude-opus-4-8`, `gpt-5-pro`,
`gpt-5.6-terra`. Осталось dry-run: `claude-opus-5` (нерабочая у провайдера),
`gpt-5.4-pro`, `gpt-5.3` (нерабочая у провайдера).**

### `gpt-5.4-pro` — раскатано 2026-08-07

Пятая модель, та же вариация бага прокси laozhang (400 `max_output_tokens`
below minimum), уже покрытая обобщённым матчингом из фикса gpt-5.6-terra —
фолбэк на apimart сработал сразу, отдельного код-фикса не потребовалось.

Живой тест на `.ru` вскрыл ДРУГОЙ баг — не в фолбэке, а в самом refund.
Сообщение `chat:3231` (reasoning-класс, ~270с генерации до ошибки apimart)
списало 55 ₽ (`flat`), упало с генеральной ошибкой, и код по всем признакам
прошёл через refund-ветку `except Exception` в `StreamMessageView.generate()`
(`api/views/chats.py`) — статус/`error_message` сообщения сохранились с
текстом, совпадающим байт-в-байт с `t_error('generation_error_generic')` — но
`BalanceTransaction(type='refund', reference='chat:3231')` в БД не появилась.
Деньги пользователя пропали без начисления и без исключения наружу.

Корень: `add_kopecks()` в `users/models.py` открывает `transaction.atomic()`
на СУЩЕСТВУЮЩЕМ соединении с БД, которое всю долгую генерацию (270+с)
простаивало без единого запроса. Если соединение за это время стало
непригодным (сетевой обрыв/протокольная ошибка на стороне БД, не
`IntegrityError`), первый же запрос внутри `add_kopecks()` падает —
`except IntegrityError` эту ошибку не ловит, она должна была прервать
выполнение `except`-блока раньше выставления `error_message`, но
на практике (воспроизвести детерминированно не удалось) сообщение всё же
сохранилось — то есть по сути редкий race в связке долгий idle + падение
именно на первом write после него. Смежная находка при диагностике: опечатка
в `LOGGING` в `config/settings.py` — ключ dictConfig называется
`disable_existing_loggers`, а не `disable_existing_handlers` (неизвестный
ключ молча игнорировался Python `logging`, оставляя дефолт
`disable_existing_loggers=True`); без явного `root` логгеры вида
`api.views.chats` уходили в root без хендлеров — `logger.error`/`.warning`
из бизнес-кода пропадали бесследно, что и мешало поймать сам баг живьём
(не путать с буферизацией stdout — `PYTHONUNBUFFERED=1` в Dockerfile уже
стоял и был ни при чём).

**Фикс (коммит `726d5bc`):** `connection.close_if_unusable_or_obsolete()` —
штатный Django-идиом, дешёвая проверка перед refund/metering в `except`- и
`finally`-путях `generate()`; если соединение протухло, следующий запрос
прозрачно откроет новое. Плюс исправлена опечатка `disable_existing_loggers`
и добавлен `root`-логгер. QA-аккаунту вручную возвращено 55 ₽ той же
командой, что и штатный refund (`add_kopecks(5500, type='refund',
reference='chat:3231')` — прошла с первого раза после фикса, тем самым
задвоив роль диагностики и настоящего возврата).

После деплоя фикса на оба инстанса живой повтор на `.ru` (новый чат 980,
статья ~3000+ слов о будущем энергетики) прошёл чисто: `spend -5500
chat:3235`, затем `overage -11000 overage:3235` (903/15044 токенов,
`source=provider`) — обе транзакции на месте, без потерь. На `.net` тест
прошёл ещё раньше (до обнаружения бага на `.ru`, независимая генерация):
доплата 8665 credits (`overage:57`), чек в интерфейсе «Long response · 6,709
tokens · extra charge 8,665 credits».

**Раскатано: `claude-fable-5`, `claude-opus-4-8`, `gpt-5-pro`,
`gpt-5.6-terra`, `gpt-5.4-pro`. Осталось dry-run только нерабочие у
провайдера модели: `claude-opus-5`, `gpt-5.3` — раскатка по плану завершена.**
