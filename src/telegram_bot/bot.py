...
def register_routers():
    ...
    dp.include_router(mybot_cmd.router)      # /mybot — персональные AI-боты (S8)
    dp.include_router(agent_cmd.router)      # /agent — Agent Mode (S9)
    dp.include_router(channel_cmd.router)    # /channel — Канал-мейкер (S11)
