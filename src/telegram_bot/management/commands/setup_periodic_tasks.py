...
TASKS = [
    ...
    {
        "name": "Биллинг: закрыть зависшие Deep Research и вернуть предоплату (каждые 10 минут)",
        "task": "aitext.tasks.reap_stuck_researches",
        "schedule": {"minute": "*/10"},
        "cron": True,
    },
    {
    ...
    },
    {
        ...
        # фактически неточны, но это существовавшая до меня особенность
        # файла, не трогаю остальные записи в рамках этой задачи).
        "name": "Мониторинг: здоровье текстовых моделей (понедельник 08:00 МСК)",
        "task": "aitext.tasks.check_model_health",
        "schedule": {"day_of_week": "1", "hour": "8", "minute": "0"},
        "cron": True,
    },
]
