...
SYSTEM_COMMITS_EN = (
    ...
    "- DATABASE RULE: if the user did NOT specify a database or said 'not decided/don't know' —\n"
    "  use localStorage/JSON files/in-memory storage. Do NOT add PostgreSQL/MySQL/MongoDB without\n"
    "  explicit user request. Always include a step for src/lib/mock-data.ts (10-20 realistic records).\n\n"
...
SYSTEM_COMMITS_RU = (
    ...
    "- ВАЖНО — база данных: если пользователь НЕ указал конкретную БД или написал 'не решил/не знаю' —\n"
    "  используй localStorage/JSON-файлы/in-memory хранение. НЕ добавляй PostgreSQL/MySQL/MongoDB\n"
    "  без явного запроса. Всегда включай шаг с src/lib/mock-data.ts (10-20 реалистичных записей).\n\n"
...
ARCHITECT_COMMITS_V3_RU = (
    ...
    "- Моки и данные — в lib/mock-data.ts (10-20 реалистичных записей), не внутри компонента.\n"
    ...
    "- БД: если пользователь не указал конкретную БД или написал 'не решил' — используй\n"
    "  localStorage/JSON. НЕ добавляй PostgreSQL/MongoDB без явного запроса.\n\n"
...
ARCHITECT_COMMITS_V3_EN = (
    ...
    "- Mocks and data in lib/mock-data.ts (10-20 realistic records), not inside components.\n"
    ...
    "- DATABASE: if user did NOT specify a DB or said 'not decided' — use localStorage/JSON.\n"
    "  Do NOT add PostgreSQL/MongoDB without explicit user request.\n\n"
