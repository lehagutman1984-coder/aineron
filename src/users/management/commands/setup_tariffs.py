...
TARIFFS = [
    ('Старт', 149, 170, 15,
     'Для знакомства с нейросетями: 170 ₽ на балансе — это ~170 ответов DeepSeek '
     'или ~40 ответов GPT-4o. Баланс не сгорает.'),
    ('Стандарт', 399, 480, 40,
     'Оптимум для повседневной работы: +20% бонус к пополнению — хватает '
     'на ~120 ответов GPT-4o или ~70 изображений Midjourney в месяц.'),
    ('Про', 899, 1150, 90,
     'Для активных пользователей: +28% бонус — примерно 95 ответов GPT-5, '
     '287 ответов GPT-4o или 13 видео Veo 3.1 Fast в месяц.'),
    # AI-секретарь (S5) входит в «Бизнес» и «Макс» — проверка по названию
    # тарифа в telegram_bot/handlers/business.py::_charge_reply
    ('Бизнес', 1290, 1700, 130,
     'AI-секретарь в Telegram Business (300 авто-ответов клиентам в месяц, '
     'сверх — 1 ₽/ответ) + 1700 ₽ на баланс (+32% бонус) — ~425 ответов GPT-4o.'),
    ('Макс', 2190, 3000, 200,
     'Самый выгодный: +37% бонус — 3000 ₽ на баланс для текста, изображений '
     'и видео + AI-секретарь в Telegram Business (300 авто-ответов в месяц).'),
]
