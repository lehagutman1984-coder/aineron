...
def handle_user_saved(sender, instance, created, **kwargs):
    ...
    try:
        if created:
            logger.info(f"✅ Создан новый пользователь: {instance.email} (ID: {instance.id})")

            # ❌ УДАЛЕНО: создание EmailAddress здесь
        ...
        logger.error(f"❌ Ошибка в handle_user_saved: {e}")
    ...
    try:
        if created:
            ...
            if not user.email_verified:
                ...
                logger.info(f"✅ Email автоматически подтвержден для {user.email} (соц. аккаунт: {provider})")
            ...
            if email_address and not email_address.verified:
                ...
                email_address.save()
                logger.info(f"✅ EmailAddress подтвержден для {user.email}")
            elif not email_address:
                logger.debug(f"ℹ️ EmailAddress для {user.email} еще не создан allauth, пропускаем")

    except Exception as e:
        logger.error(f"❌ Ошибка в handle_social_account_saved: {e}")
    ...
    try:
        if created:
            ...
            if other_users_count > 0:
                logger.warning(f"⚠️ Обнаружено несколько пользователей с IP {instance.ip_address}: "
                               f"{instance.user.email} и еще {other_users_count} пользователь(я)")

    except Exception as e:
        logger.error(f"❌ Ошибка в handle_user_ip_saved: {e}")
            ...
            try:
                ...
                if old_instance.shadow_banned != instance.shadow_banned:
                    if instance.shadow_banned:
                        logger.warning(f"⚠️ Пользователю {instance.email} назначен теневой бан")
                    else:
                        logger.info(f"✅ С пользователя {instance.email} снят теневой бан")

                # Проверяем подтверждение email
                if not old_instance.email_verified and instance.email_verified:
                    logger.info(f"✅ Email {instance.email} подтвержден")

            except CustomUser.DoesNotExist:
                pass

    except Exception as e:
        logger.error(f"❌ Ошибка в handle_user_pre_save: {e}")
        ...
        if instance.pk:
            try:
                ...
                if old_instance.last_login != instance.last_login and instance.last_login:
                    ...
                    logger.debug(f"📊 Зафиксирован вход пользователя {instance.email}")

            except CustomUser.DoesNotExist:
                pass

    except Exception as e:
        logger.error(f"❌ Ошибка в handle_user_login: {e}")