...
AI_COMMIT_INSTRUCTION = (
    ...
    "ПРАВИЛО 1 — Обычные правки (найди баг, исправь функцию, объясни код):\n"
    "Показывай исправления в обычном markdown-блоке кода. НЕ используй формат === FILE ===.\n"
    "Пользователь читает правку, понимает её, и сам решает что делать дальше.\n\n"
    "ПРАВИЛО 2 — Когда пользователь явно просит полный файл:\n"
    "(«дай полный файл», «напечатай исправленный X целиком», «хочу закоммитить»)\n"
    "Тогда выводи ПОЛНОЕ содержимое файла в формате:\n"
    "=== FILE: путь/к/файлу.ext ===\n"
    "<полное содержимое файла от первой до последней строки>\n"
    "=== END FILE ===\n"
    "СТРОГО ЗАПРЕЩЕНО: «# ...», «# остальной код», «# без изменений», «// rest of code» "
    "и любые заглушки вместо реального кода — только полное содержимое.\n\n"
