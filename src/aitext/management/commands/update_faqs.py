...
GLOBAL_FAQS = [
    ...
    {
        "question": "Как работают звёзды и подписка на платформе?",
        "answer": (
            "Звёзды — внутренняя валюта aineron.ru. Каждое сообщение к нейросети стоит "
            "1–5 звёзд в зависимости от модели. Звёзды не сгорают и не ограничены по времени. "
            "Подписка добавляет пакет звёзд и открывает безлимитный доступ к базовым моделям."
        ),
        "order": 2,
    },
    {
    ...
    },
    {
        ...
        "answer": (
            "Да. Новым пользователям начисляются стартовые звёзды (10 штук), которых хватает "
    ...
    },
    {
        ...
        "answer": (
            "Перейдите в «Кабинет» → «Пополнение». Доступна покупка звёзд поштучно или "
            "подписки со скидкой. Принимаем карты Visa / MasterCard / Мир, ЮMoney, СБП. "
