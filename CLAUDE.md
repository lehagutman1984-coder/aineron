...
## Миграция фронтенда на Next.js (план)

Текущий фронтенд: Vanilla JS + CSS. Планируется поэтапная миграция на **Next.js 14 App Router**.

**Целевой стек фронтенда:**
| Слой | Технология |
|------|-----------|
| Framework | Next.js 14 App Router |
| Язык | TypeScript |
| Стейт | Zustand |
| Запросы | React Query (TanStack Query v5) |
| Стили | Tailwind CSS v4 |
| Иконки | **Lucide React** (основной набор) |
| Расположение | `frontend/` в корне репозитория |

**Архитектура маршрутов:**
- `/` (лендинг) — SSG (статика, SEO)
- `/catalog/` — SSR (список нейросетей с SEO)
- `/catalog/[slug]/` — SSR (детальная страница модели + JSON-LD)
- `/chat/[networkSlug]/` — Client Component (React Query polling)
- `/account/` — Client Component (баланс, тарифы, история)
- `/docs/` — SSG (документация API)
- `/blog/` — SSR + ISR
- `/admin/` — остаётся Django Admin, не мигрируется

**Backend при миграции:**
- Django переходит в режим чистого API: DRF + `/api/v1/`
- Аутентификация: JWT (djangorestframework-simplejwt) вместо сессий для Next.js
...
/admin/ → Django (порт 8000)
/media/ → static files напрямую
```
...
### Текущий фронт (Django шаблоны, до миграции)
- Новые шаблоны `.html` и JS-файлы создавать без эмодзи
...
Все 6 фаз ROADMAP **завершены**. Актуальный план развития — конкурентная стратегия выхода в топ-3 России — в файле `COMPETITIVE_PLAN.md` в корне репозитория.
...
- **ФАЗА 5** — SEO-блог, Analytics — ЗАВЕРШЕНА (~70%)
- **ФАЗА 6** — Embeddings, Audio, Batch, Webhooks, Audit, Status — ЗАВЕРШЕНА

### Следующий приоритет (COMPETITIVE_PLAN.md)
Три спринта для выхода в топ-3:
- **Sprint 1 (1–2 дня)** — Chat sidebar, Markdown рендеринг, Copy/Regenerate, Dark Mode, Starter prompts
- **Sprint 2 (2–14 дней)** — Загрузка файлов, Model Arena, Промпт-библиотека, Веб-поиск, Projects, Аналитика, Лендинг 2.0
- **Sprint 3 (1–3 месяца)** — Telegram-бот, PWA/Mobile, Голос, Persistent Memory, SEO-машина
