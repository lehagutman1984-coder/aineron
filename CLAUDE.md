...
Подробный план развития проекта по фазам и коммитам — в файле `ROADMAP.md` в корне репозитория.

Фазы:
- **ФАЗА 0** — SEO и zero-emoji (текущий стек, быстрые победы)
- **ФАЗА 1** — DRF API-слой + OpenAI-совместимые эндпоинты + API-ключи
- **ФАЗА 2** — Инфраструктура Next.js (проект, конфиг, Auth, DRF)
- **ФАЗА 3** — Миграция страниц на Next.js (лендинг → каталог → чат → аккаунт)
- **ФАЗА 4** — B2B (счета, оферта, командные аккаунты)
- **ФАЗА 5** — SEO-доминирование (блог, статьи, программатик-контент)
- **ФАЗА 6** — Advanced (embeddings API, дашборд аналитики, enterprise)
